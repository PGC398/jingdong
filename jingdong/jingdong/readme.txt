京东爬虫
===
京东scrapy爬虫,依据搜索引擎实现指定商品爬虫并将商品参数与评论下载入库。
依赖
===
	scrapy
	redis
...
分布式爬虫配置
===
在jingdong/jingdong/settings.py中
```Python
...
MONGODB_SERVER = "localhost" #此处配置redis,服务器使用127.0.0.1/localhost 即可,配置分布式时，需要从机在此处填主机的ip地址,即集中存储数据于服务器上
...
REDIS_HOST = '127.0.0.1' #此处配置redis,服务器使用 127.0.0.1/localhost 即可,配置分布式时，需要从机在此处填主机的ip地址
REDIS_PORT= 6379

```

注:配置好分布式爬虫后,开启爬虫前,首先要清空redis数据,然后在服务器redis数据库中插入初始url,插入格式如下：
